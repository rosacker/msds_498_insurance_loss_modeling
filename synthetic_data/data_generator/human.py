...
class human:
  def __init__(self, household, target_age, target_gender = None, upbringing_score = None):
    ...
    self.gender = random.choices(['m', 'f'], weights = [0.5,0.5])[0] if target_gender is None else target_gender
    self.married = False   
    self.education = 'uneducated' 
    self.job_class = 0

    self.age_licensed = random.choices([16, 17, 18, 19, 20], weights = [0.7, 0.1, 0.1, 0.05, 0.05])[0]
    ...
    self.upbringing_score = upbringing_score if upbringing_score is not None else random.normalvariate(0, 3)
    self.risk_mitigation_score = self.upbringing_score + random.normalvariate(0, 0.5)
  ...
  def evaluate_job(self):
    ...
    if change_up_index > random.uniform(0, 1):    
      ...
      new_index = max(min(current_index + movement, len(allowed_range)-1), 0)
